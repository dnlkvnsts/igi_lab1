...
class CarDataAnalyzer(PandasTaskBase, AutoStatsMixin):
    ...
    def get_full_dataset(self):
        ...
        file_path = os.path.join(base_path, 'Automobile_data.csv')

        
        df = pd.read_csv(file_path, na_values='?')
        
       
        df = df[['make', 'num-of-cylinders', 'city-mpg', 'price']]
        ...
        df['cylinders'] = df['cylinders'].map(cyl_map)
        
        return df
    
    
    def show_dataset_info(self, df):
        """
        'Getting information about the dataframe (by each parameter)'
        """
        print("\n--- General Information ---")
        print(df.info()) 
        print("\n--- Data Types ---")
    ...
    def analyze_cylinders_price(self, df):
        temp_df = df.dropna(subset=['cylinders'])
        
        max_cyl = temp_df['cylinders'].max()
       
        avg_price = temp_df[temp_df['cylinders'] == max_cyl]['price'].mean()
        
        return int(max_cyl), round(avg_price, 2)
